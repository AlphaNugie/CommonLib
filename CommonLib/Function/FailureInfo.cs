...
namespace CommonLib.Function
{
    ...
    public class FailureInfo
    {
        ...
        public static string[] GetFailureInfoArray(Exception ex, string info = "出现异常", bool usingExcepMsg = true, IEnumerable<string> extraInfos = null)
        {
            ...
            {
                usingExcepMsg ? string.Format("{0}：{1}", info, ex.Message) : info, //是否需要异常信息
                //string.Format("出现异常的方法：{0}", ex.TargetSite.ToString()),
                //string.Format("方法所在的类：{0}", ex.TargetSite.ReflectedType.FullName), //命名空间+类名
                string.Format("出现异常的方法：{0}", ex.TargetSite == null ? "null" : ex.TargetSite.ToString()),
                string.Format("方法所在的类：{0}", ex.TargetSite == null ? "null" : ex.TargetSite.ReflectedType.FullName), //命名空间+类名
                ex.ToString()
            };
