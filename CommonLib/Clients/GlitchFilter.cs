    ...
    public class GlitchFilter
    {
        ...
        public void PushValue(ref double value)
        {
            ...
            if (LongDistAvoidEnabled)
            {
                ...
                //只在距离达到远距值的时候处理，假如达到远距值的次数未超过限定值，则以上一个输出值代替观测值与初步处理值，否则
                if (isLongDist)
            ...
            {
                //double prevo = _storage.First(), diff = temp - prevo, sum = 0;
                //double prevo = _storage.First(), diff = ObsVal - prevo, sum = 0; //上一个观测值，当前观测值与上一个观测值的差值，队列中所有值差值的和（下一个减上一个）
                ...
                //temp = Math.Abs(diff) < StepWidth * aver || (FixedGapEnabled && Math.Abs(diff) < FixedGapThres) ? temp : _prevp + StepWidth * aver * Math.Sign(diff);
                //temp = Math.Abs(diff) <= StepWidth * aver ? temp : _prevp + StepWidth * aver * Math.Sign(diff);
