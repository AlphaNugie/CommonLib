...
namespace OpcLibrary.Controls.Forms
{
    ...
    public partial class FormOpcConfig : Form
    {
        ...
        private void Button_Import_Click(object sender, EventArgs e)
        {
            ...
            DataRowCollection rowCol = ((DataTable)dataGridView_Main.DataSource).Rows;
            //var rowList = rowCol.Cast<DataRow>().ToList();
            var itemIdList = rowCol.Cast<DataRow>().Select(row => row.Convert("ITEM_ID", string.Empty)).ToList();
            items.ForEach(item =>
            {
                if (!itemIdList.Contains(item.ItemId))
                {
                    //按数据库字段的顺序排列
