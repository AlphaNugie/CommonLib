...
namespace CommonLib.DataUtil
{
    ...
    public static class OracleExtension
    {
        ...
        /// </summary>
        /// <param name="provider">Oracle基础操作类对象，为空时返回null</param>
        /// <param name="tableName">表名，为空时返回null</param>
        /// <param name="columnNames">给定的数据库字段列表，假如为空则获取所有字段</param>
        /// <param name="message"></param>
        /// <returns></returns>
        public static List<OracleColumnMapping>? GetColumnMappings(this OracleProvider? provider, string tableName, IEnumerable<string>? columnNames, out string message)
        {
            message = string.Empty;
            if (provider == null || string.IsNullOrWhiteSpace(tableName))
                return null;
            List<OracleColumnMapping>? columns = null;
            string sqlString = string.Format(@"
SELECT utc.column_name, utc.data_type, utc.data_length, utc.nullable, utc.data_default, ucc.comments
FROM user_tab_columns utc
JOIN user_col_comments ucc ON utc.table_name = ucc.table_name AND utc.column_name = ucc.column_name
WHERE utc.table_name = '{0}';", tableName);
            ...
            columns = table.Rows.Cast<DataRow>().Select(dataRow => new OracleColumnMapping(dataRow)).ToList();
        ...
        /// 根据给定的表名判断此表是否存在
        /// </summary>
        /// <param name="provider">Oracle基础操作类对象，为空时返回false</param>
        /// <param name="tableName">表名，为空时返回false</param>
        /// <param name="table">假如此表存在，输出表内容</param>
        /// <returns></returns>
        public static bool TableExists(this OracleProvider? provider, string tableName, out DataTable? table)
        {
            table = null;
            if (provider == null || string.IsNullOrWhiteSpace(tableName))
                return false;
        ...
        public static bool CheckForTable(this OracleProvider? provider, string tableName, IEnumerable<OracleColumnMapping> columnsMustHave, out DataTable? table, out string errorMessage)
        {
            table = null;
            ...
            if (provider == null || string.IsNullOrWhiteSpace(tableName))
            {
                errorMessage = "给定的Oracle操作对象或表名为空";
                return false;
            }
            //假如表存在，返回true
            if (provider.TableExists(tableName, out table))
                return true;
            if (columnsMustHave == null || !columnsMustHave.Any())
            {
                errorMessage = "表不存在，因给定列为空，将不进行新增操作";
                return false;
            }
            string columns = string.Join(string.Empty, columnsMustHave.Select(column => string.Format("\r\n  {0},", column.GetStructure()))).TrimEnd(',');
            List<string> sqlStrings = new() { string.Format(@"
create table {0}
({1}
);", tableName, columns) };
            sqlStrings.AddRange(columnsMustHave.Where(col => !string.IsNullOrWhiteSpace(col.Comment)).Select(col => $"comment on column {tableName}.{col.ColumnName} is '{col.Comment}';"));
            provider.ExecuteSqlTrans(sqlStrings);
        ...
        /// <returns></returns>
        public static bool CheckForTableColumns(this OracleProvider? provider, string tableName, IEnumerable<OracleColumnMapping> columnsMustHave, out string message)
        {
            message = string.Empty;
            if (columnsMustHave == null || !columnsMustHave.Any())
                return true;
            ...
            if (!tableExists || !string.IsNullOrWhiteSpace(message))
                return tableExists;
            //全部转为小写以进行比对
            List<string> currCols = table.Columns.Cast<DataColumn>().Select(column => column.ColumnName.ToLower()).ToList(), fields = new(), sqls = new();
        ...
        /// </summary>
        /// <param name="provider">Oracle基础操作类对象</param>
        /// <param name="tableName">表名</param>
        /// <param name="orderby">排序字段，假如为空则不排序</param>
        /// <returns></returns>
        public static DataTable? GetAllRecords(this OracleProvider? provider, string tableName, string orderby)
        {
            if (provider == null || string.IsNullOrWhiteSpace(tableName))
                return null;
